...
(function () {

	{
		class Robot {
		  constructor(color, light, size, x, y, struct) {
			...
			for (const p of struct.points) {
			  this.points.push(new Robot.Point(size * p.x + x, size * p.y + y - 800, p.f));
			}
			// ---- create links ----
		...
		dancers.push(new Robot(2*360/7,80,7,(1+2)*canvas.width/9,canvas.height*ground-1000,struct));//DEFAULT EMPTY LINE
		// ---- warm up ----
		run();
	  }
})();
